...
// Derived from the curriculum source of truth so certificates can never drift.
const certifications = TRACKS.filter((t) => t.code !== 'DT-101').map((t) => ({
  id: t.slug,
  icon: t.icon,
  title: CERT_NAMES[t.code] ?? `Certified ${t.name}`,
  track: t.name,
  accent: t.accent,
  isAdvanced: !!t.isAdvanced,
  description: `Awarded to students who complete the ${t.name} track and prove the skills below through a real capstone project — not an exam.`,
  skills: t.outcomes,
  capstone: t.weeks[t.weeks.length - 1]?.deliverable ?? '',
  roles: t.careerPaths,
}));

const CERT_COUNT = certifications.length;
      ...
      <main style={{ paddingTop: '68px' }}>
        ...
        <section style={{ padding: '5rem 2.5rem', maxWidth: '1180px', margin: '0 auto' }}>
          <h2 style={{
            ...
            All <span style={{ color: 'var(--cyan)' }}>{CERT_COUNT} Certificates</span>
          </h2>
