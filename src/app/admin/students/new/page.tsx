...
export default function NewStudentPage() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', track: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (res.ok) {
      setSuccess(`${form.full_name} has been enrolled. An invite email has been sent to ${form.email}.`);
      setForm({ full_name: '', email: '', track: '' });
    } else {
    ...
    <div style={{ maxWidth: '500px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div><label style={labelStyle}>Track *</label>
          <select style={{ ...inputStyle, cursor: 'pointer' }} value={form.track} onChange={e => set('track', e.target.value)} required onFocus={e => (e.target.style.borderColor = 'var(--cyan-border)')} onBlur={e => (e.target.style.borderColor = 'var(--border)')}>
