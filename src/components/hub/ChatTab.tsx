...
export default function ChatTab({ studentId, studentName }: { studentId: string; studentName: string }) {
  ...
  // Polling — full refetch every 2 seconds; catches new messages AND updates (pin/delete)
  useEffect(() => {
    if (!activeChannel) return;
    const id = setInterval(async () => {
      ...
      setMessages(prev => {
        ...
        // Update existing messages (e.g. pin toggle) and append new ones
  ...
  return (
    <div style={{ display: 'flex', gap: '0', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden', height: '600px' }}>
              ...
              <div key={type} style={{ marginBottom: '0.75rem' }}>
                ...
                {group.map(ch => (
                  <button key={ch.id} onClick={() => setActiveChannel(ch)} style={{
                    ...
                    padding: '0.5rem 0.6rem', borderRadius: '7px', border: 'none', cursor: 'pointer',
                    background: activeChannel?.id === ch.id ? 'rgba(0,200,255,0.1)' : 'transparent',
                    color: activeChannel?.id === ch.id ? 'var(--cyan)' : 'var(--text)',
                    fontFamily: 'var(--font-body)', fontSize: '0.82rem', textAlign: 'left',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ padding: '0.85rem 1.25rem' }}>
          <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'flex-end' }}>
            ...
            />
            <button onClick={send} disabled={sending || !input.trim() || !activeChannel} style={{ padding: '0.6rem 1.1rem', background: (input.trim() && activeChannel) ? 'var(--cyan)' : 'rgba(0,200,255,0.15)', color: (input.trim() && activeChannel) ? '#070D1A' : 'var(--muted)', border: 'none', borderRadius: '8px', fontFamily: 'var(--font-head)', fontWeight: 700, fontSize: '0.85rem', cursor: (input.trim() && activeChannel) ? 'pointer' : 'not-allowed', transition: 'all 0.15s', flexShrink: 0 }}>
              Send
            </button>
          </div>
          <div style={{ fontSize: '0.68rem', color: 'var(--muted)', marginTop: '0.35rem' }}>Enter to send · Shift+Enter for new line</div>
